...
\date{\today}
    \item \textbf{gateway}: Microservicio Spring Cloud Gateway (reemplaza NGINX). Realiza balanceo, ruteo dinámico, control de acceso, validación de token JWT, reglas de defensa básica, propagación de Correlation ID y cuenta con pruebas automatizadas integradas en el pipeline CI/CD.
\begin{document}
\maketitle
...
Repositorio: \url{https://github.com/M4gic1540/GrupoCordillera.git}
    ...
    \item 1 gateway (Spring Cloud Gateway) como punto de entrada.
...
Beneficio: el servicio de autenticación concentra reglas de negocio (registro, login, refresh, perfil) sin mezclar SQL ni detalles de infraestructura. Además, la integración con sistemas externos se desacopla mediante el patrón Factory Method, facilitando la extensión de conectores.
...
Actualmente se utilizan 3 ramas principales, alineadas y sincronizadas tras los últimos merges:
...
En \texttt{authservice} se observa una aplicacion madura del patron:
    \item Balanceo y ruteo dinámico para authservice, data-ingestion-service y kpi-engine.
    \item Filtro global de autenticación y autorización JWT.
    \item Limitación de tasa (rate limiting) por IP.
    \item Reglas WAF básicas para bloquear patrones de inyección y exploración maliciosa.
    \item Propagación de \texttt{X-Correlation-ID} para observabilidad transversal.
    \item Pruebas automatizadas (más de 100 tests) ejecutadas en Jenkins.
...
Beneficio: separa definiciones de KPI y resultados calculados, mejorando trazabilidad y mantenibilidad.
    ...
    \begin{itemize}[leftmargin=*]
        \item Integración de pruebas automatizadas en gateway, authservice, data-ingestion-service y kpi-engine (más de 400 tests en total).
        \item Validación de cobertura de pruebas y calidad de código en cada build.
        \item Uso de Maven Wrapper en todos los microservicios para portabilidad y consistencia en CI/CD.
    \end{itemize}
...
\end{itemize}
    ...
    \item Integración de conectores externos con Factory Method en authservice.
    \item Pruebas de integración, E2E, performance y seguridad en gateway, data-ingestion-service y kpi-engine.
...
\end{itemize}
\textbf{Nota:} Tras los últimos cambios, se completaron los siguientes puntos:
\begin{itemize}[leftmargin=*]
    \item Implementación del patrón Factory Method para conectores externos en authservice (RNF-14).
    \item Pruebas automatizadas en todos los microservicios (más de 400 tests).
    \item Integración y alineación de ramas feature, developer y main.
    \item Uso de Spring Cloud Gateway como gateway principal.
    \item Uso de Maven Wrapper y pipeline CI/CD robusto.
\end{itemize}
...
\section{Gateway (Spring Cloud Gateway) y Control de Trafico}
El gateway implementa funciones clave como microservicio Spring Boot:
\begin{itemize}[leftmargin=*]
    \item Balanceo y ruteo dinámico para authservice, data-ingestion-service y kpi-engine.
    \item Filtro global de autenticación y autorización JWT.
    \item Limitación de tasa (rate limiting) por IP.
...
Esta capa mejora seguridad, resiliencia y trazabilidad operacional del sistema. El cambio de NGINX a Spring Cloud Gateway permite mayor flexibilidad, integración con el ecosistema Spring y automatización de pruebas.
...
RNF-10 & Trazabilidad por Correlation ID. & Completo & Filtro de correlación + propagación en Spring Cloud Gateway. \\
